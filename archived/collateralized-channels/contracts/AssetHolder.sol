...
import "./perun-eth-contracts/contracts/Sig.sol";
...
abstract contract AssetHolder {
    ...
    external onlyAdjudicator {
        ...
        // The following segment redistributes the deposited assets according to
        // the concluded channel balances.
        // We do not redistribute assets until the channel is properly funded to
        // ensure asset security during the funding phase.
        // We do not redistribute assets if we observe all-zero balances to
        // allow for channels with a custom balance and settlement logic.
        // We allow overfunding channels, who overfunds looses their funds.
        if (sumHeld >= sumOutcome && sumOutcome > 0) {
            for (uint256 i = 0; i < parts.length; i++) {
    ...
    function withdraw(WithdrawalAuth calldata authorization, bytes calldata signature) external {
        ...
        require(holdings[id] >= authorization.amount, "insufficient assets for withdrawal");
