...
func createFunder(cb channel.ContractBackend, account accounts.Account, assetHolder common.Address) *channel.Funder {
	ethDepositor := new(channel.ETHDepositor)
	accounts := map[channel.Asset]accounts.Account{wallet.Address(assetHolder): account}
	depositors := map[channel.Asset]channel.Depositor{wallet.Address(assetHolder): ethDepositor}
	return channel.NewFunder(cb, accounts, depositors)
}
